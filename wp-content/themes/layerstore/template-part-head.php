		...
		<div class="header-login col-sm-4 col-xs-4 col-sm-pull-4">
			<?php if ( class_exists( 'WooCommerce' ) && get_theme_mod( 'my-account-link', 1 ) == 1 ) { // Login Register  ?>
				<?php if ( is_user_logged_in() ) { ?>
					<a href="<?php echo get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ); ?>" title="<?php _e( 'My Account', 'layerstore' ); ?>"><?php _e( 'My Account', 'layerstore' ); ?></a>
				<?php } else { ?>
					<a href="<?php echo get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ); ?>" title="<?php _e( 'Login / Register', 'layerstore' ); ?>"><?php _e( 'Login / Register', 'layerstore' ); ?></a>
				<?php } ?>
			<?php } ?>
		</div>
		<?php // Shopping Cart   ?>
			...
			<nav id="site-navigation" class="navbar navbar-inverse" role="navigation">
				...
				<div class="navbar-header">
					<button type="button" class="navbar-toggle" data-toggle="collapse" data-target=".navbar-1-collapse">
						<span class="sr-only"><?php _e( 'Toggle navigation', 'layerstore' ); ?></span>
