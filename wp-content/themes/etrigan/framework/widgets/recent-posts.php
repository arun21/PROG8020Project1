...
class etrigan_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'etrigan_rp', // Base ID
			__('Etrigan Recent Posts with Thumbnails', 'etrigan'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'etrigan' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'etrigan' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'etrigan' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:','etrigan' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />

		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:', 'etrigan' ); ?></label>
...
add_action( 'widgets_init', 'register_etrigan_widget' );
function register_etrigan_widget() {
    register_widget( 'etrigan_Recent_Posts' );
}
